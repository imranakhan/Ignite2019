...
function DownloadSession($sessionObject, $sessionSearchCount, $directory) {
    if (($sessionObject.slideDeck.Length -ne 0) -or ($sessionObject.downloadVideoLink.Length -ne 0)) {
        ...
        $longFolderName = "$code - $title" -Replace "^\W";
        $longFolderName = CleanFilename $longFolderName;
        $folderName = $longFolderName.Substring(0, [System.Math]::Min(150, $longFolderName.Length));
        ...
        #Create directory.
        $folder = Join-Path -Path $directory -ChildPath $folderName;   # "("+$s.sessionCode+") "+ $s.title;
        if (-not (Test-Path $folder)) {
            Write-Host "Folder ($folder) doesn't exist. Creating it..."  ;
            New-Item $folder -type directory | Out-Null;
        }

        $videoFile = "$directory\$folderName\$code.mp4";
        $slideFile = "$directory\$folderName\$code.pptx";
